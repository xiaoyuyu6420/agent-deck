...
| `Accept { i }` | 可选 slot | 批准 slot `i`（或焦点 slot）对应的 waiting Session 的当前审批请求 | ⏳ unsupported |
| `Reject { i }` | 可选 slot | 拒绝对应 Session 的当前审批请求 | ⏳ unsupported |
| `Stop { i }` | 可选 slot | 中断对应 Session 的当前回合 | ⏳ unsupported |
| `StopAll` | — | 中断所有 running Session 的当前回合 | ⏳ unsupported |
...
| **Phase 1** | Codex 裁决实现：`serverRequest/resolved` + `turn/interrupt`，observer 携带 requestId，trait 加 `dispatch` | 无（通道已就绪） | 未开始 |
